...
/// Binds an IPv4 loopback listener on a specific port. Used for OAuth2 authorization code
/// grant flow redirect callbacks, where the redirect URI must exactly match the port
/// registered in the OAuth provider's application settings.
pub async fn tcp_listen_loopback(port: u16) -> io::Result<TcpListener> {
