...
use theseus::prelude::tcp_listen_loopback;
use tokio::sync::{broadcast, oneshot};

/// Fixed loopback port used for OAuth2 redirect callbacks. This must match the redirect URI
/// registered in the OAuth provider's application settings (e.g. `http://127.0.0.1:34137`).
pub const AUTH_CODE_REPLY_PORT: u16 = 34137;
...
/// on the loopback interface on [`AUTH_CODE_REPLY_PORT`]. The caller can know the bound socket
/// address by listening on the counterpart channel for `listen_socket_tx`.
///
...
pub async fn listen(
    ...
    let listener = match tcp_listen_loopback(AUTH_CODE_REPLY_PORT).await {
        Ok(listener) => {
            listen_socket_tx
